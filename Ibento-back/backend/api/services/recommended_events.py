...
def obtener_eventos_recomendados(preferencias_usuario, eventos, eventquery):

    # print("Eventos:")
    # print(eventos)

    #Generamos los vectores de los eventos y el usuario
    vectores_eventos = crear_vectores_eventos(eventos, ticketmaster_elements)
    vector_usuario = tags_a_vector(preferencias_usuario, ticketmaster_elements)

    similitudes = cosine_similarity(
        [vector_usuario], list(vectores_eventos.values())
    )[0]
    ...
    #Limitar a los 20 eventos mas relevantes
    eventosLimitados = eventos_filtrados[:20]

    #Envia unicamente los eventos y no la similitud
    eventos_recomendados = [item[0] for item in eventosLimitados]

    q_objects = Q()
    for title in eventos_recomendados:
        q_objects |= Q(title=title)

    eventos_recomendados_objects = eventquery.filter(q_objects)

    return eventos_recomendados_objects
