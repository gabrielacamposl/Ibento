...
const Verificar = () => {
    ...
    const handleSavePreferences = async () => {
        ...
        try {
            ...
            console.log("Enviando preferencias al servidor:", payload);

            // Enviar directamente a la base de datos
            const response = await api.post("intereses-respuestas/", payload);
            console.log("Respuesta del servidor:", response.data);

            // Verificar respuesta exitosa
            if (response.status === 200 || response.status === 201) {
                showSuccess("Perfil completado! Bienvenido a Ibento.");
                
                // Guardar las preferencias en estado local para referencia
                setSavedPreferences(payload);
                
                // Navegar después de un breve delay para mostrar mensaje de éxito
                setTimeout(() => {
                    navigate("../eventos");
                }, 2000);
            } else {
            ...
            showError(`Error al guardar preferencias: ${errorMessage}`);
        } finally {
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                {answers.map((answer, i) => {
                                                    ...
                                                    return (
                                                        <button
                                                            key={i}
                                                            className={`p-2 rounded-2xl border-2 transition-all duration-300 font-light text-sm ${isSelected
