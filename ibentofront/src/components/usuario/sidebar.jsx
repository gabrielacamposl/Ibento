...
export default function PositionDemo({ isWebVersion = false }) {
    ...
    async function handleUnblockUser(id){
        ...
        <div className="z-1 w-full flex justify-center items-center z-0">
            ...
            {/* Botón para versión web */}
            {isWebVersion && (
                <button
                    onClick={() => setVisibleRight(true)}
                    className="relative p-3 bg-white/60 backdrop-blur-sm border border-gray-200 hover:bg-white/80 shadow-md hover:shadow-lg rounded-2xl transition-all duration-300 transform hover:scale-105 group"
                    title="Configuración"
                    aria-label="Abrir menú de configuración"
                >
                    <svg 
